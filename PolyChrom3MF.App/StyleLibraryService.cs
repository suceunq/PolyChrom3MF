...
public sealed class StyleLibraryService
{
    ...
    public PolyStyleData Load(string path)
    {
        ...
        using (var destination = new FileStream(motifPath + ".tmp", FileMode.Create, FileAccess.Write, FileShare.None)) source.CopyTo(destination);
        File.Move(motifPath + ".tmp", motifPath, true);
