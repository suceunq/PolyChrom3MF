...
public class ThreeMfTests
{
    ...
    [Fact] public void Projet_conserve_affectations() { var source = Sample(2); var d = new ThreeMfService().Read(source); var proposals = new PaletteService().Create(d); proposals[0].Assignments[1] = 0; proposals[0].TriangleAssignments[0][0] = 3; var path = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".poly3mf"); var service = new ProjectService(); service.Save(path, d, proposals, 0, 1, 2, 3, funMode: true, colorCount: 8); var loaded = service.Load(path); Assert.Equal(0, loaded.Assignments[0][1]); Assert.Equal(3, loaded.TriangleAssignments![0][0][0]); Assert.Equal(3, loaded.Zoom); Assert.True(loaded.FunMode); Assert.Equal(8, loaded.ColorCount); }
    ...
    [Fact] public void Analyse_une_release_github_securisee() { var update = UpdateService.ParseRelease("{\"tag_name\":\"v2.4.1\",\"html_url\":\"https://github.com/suceunq/PolyChrom3MF/releases/tag/v2.4.1\",\"assets\":[{\"name\":\"PolyChrom3MF_Setup_x64.exe\",\"browser_download_url\":\"https://github.com/suceunq/PolyChrom3MF/releases/download/v2.4.1/PolyChrom3MF_Setup_x64.exe\",\"digest\":\"sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef\",\"size\":123456}]}"); Assert.Equal(new Version(2, 4, 1), update.Version); Assert.Equal("v2.4.1", update.Tag); Assert.Equal(123456, update.Size); }
